    ...
    persist(
        (set, get) => ({
            ...
            addTask: async (task) => {
                ...
                const newTask = { ...task, user_id: user.id, completed: false };

                set((state) => ({ tasks: [...state.tasks, { ...newTask, id: tempId }] }));
            ...
            toggleTask: async (id) => {
                ...
                set((state) => ({
                    tasks: state.tasks.map(t => t.id === id ? { ...t, completed: newStatus, completed_at: completedAt } : t)
                }));
                await supabase.from('tasks').update({ completed: newStatus, completed_at: completedAt }).eq('id', id);
            },
            deleteTask: async (id) => {
            ...
            addHabit: async (name, type = 'permanent') => {
                ...
                if (data) {
                    set((state) => ({ habits: state.habits.map(h => h.id === tempId ? data : h) }));
                }
            },
            ...
            toggleHabit: async (id) => {
                ...
                if (newStatus) {
                    ...
                    await supabase.from('habit_logs').upsert(newLog);
                } else {
                ...
                await supabase.from('habits').update({ completed: newStatus, streak: updatedStreak, last_completed_at: lastCompleted }).eq('id', id);
            ...
            dailyReset: async () => {
                ...
                const updatedHabits = habits.map(h => {
                    if (h.type === 'permanent') {
                        const lastDate = h.last_completed_at?.split('T')[0];
                        if (lastDate !== today) {
                ...
                // We don't necessarily need to sync this to DB immediately as it's a UI reset 
                // But it's safer to update the 'completed' state in DB too.
                await supabase.from('habits').update({ completed: false }).eq('user_id', user.id).eq('type', 'permanent').neq('last_completed_at', today);
            },
